...
import type { ApiService } from "../../../core/api.service";
...
/**
 * Holds all state for a single open spec: its data plus per-tab UI state
 * (selection, filters, layout, list search/sort). One instance per open tab,
 * so each tab keeps its own state when the user switches between them.
 *
 * Plain class (not an Angular service): `signal()`/`computed()` work outside the
 * injection context, and the manager creates instances with `new SpecTab(...)`.
 */
export class SpecTab {
	...
	async approve(hosts?: string[]): Promise<void> {
		try {
			const { data, error } = await this.api.approveSpec(this.id, hosts);
			if (error || !data) {
	...
	async load(): Promise<void> {
		...
		try {
			const { data, error } = await this.api.getSpec(this.id);
			if (error) {
